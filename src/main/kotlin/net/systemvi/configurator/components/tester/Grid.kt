...
@Composable fun Grid(keymap: KeyMap, keycapComponent: @Composable (String, Boolean, Boolean)->Unit, oneUSize:Int=50) {
            ...
            filteredItems.zip(filteredItems.indices).forEach { (row, j) ->
                row.zip(row.indices).forEach { (item, i) ->
                    ...
                    Box(
                        ...
                        keycapComponent(
                            key.name,
                            viewModel.wasDownKeys.contains(key),
                            viewModel.currentlyDownKeys.contains(key)
                        )
                    }
